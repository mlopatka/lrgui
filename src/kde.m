function [bandwidth,density,eval_point,cdf]=kde(model_data,eval_point,n)
...
%
R=abs(eval_point(end)-eval_point(1));
dx=R/(n-1); 
%eval_point=MIN+[0:dx:R]; 
N=length(unique(model_data));
%bin the data uniformly using the grid defined above;
initial_data=histc(model_data,eval_point)/N;  
...
[~,~,bandwidth]= ksdensity(model_data,1);
if nargout==0
    figure(1), plot(eval_point,density)
end
% for cdf estimation
if nargout>3
