            ...
            for i = 1:size(h.cb_features,2)
                ...
                if(get(h.cb_features(j(i)),'value')==1&&std(feature_data(:,j(i)))~=0&&strcmp(get(h.p11,'string'),'Cancel')&&(sum(cell2mat(get(h.cb_features,'value')))>1))
                    ...
                    if((strcmp(distribution_same,'KDE'))||(strcmp(distribution_diff,'KDE')))
                        ...
                        [~, parameters_same] = fit_distrib_to_data(distance_same,b_t,distribution_same);
                        [~, parameters_diff] = fit_distrib_to_data(distance_diff,b_t,distribution_diff);
                    else
                    ...
                    if(str2double(get(h.t(10),'string'))>pfp&&str2double(get(h.t(11),'string'))>pfn)
                        set(h.t(10),'string',num2str(pfp));
                        set(h.t(11),'string',num2str(pfn));
                        set(h.t(12),'string',num2str(cllr));
                        changes_0 = 1;
                    elseif((str2double(get(h.t(10),'string'))+str2double(get(h.t(11),'string')))>pfn+pfp+0.00001) % the +0.000001 is due to an error that can occur which makes that it cycles indefinetly (round off error)
                        set(h.t(10),'string',num2str(pfp));
                        set(h.t(11),'string',num2str(pfn));
                        set(h.t(12),'string',num2str(cllr));
                        changes_0 = 1;
