...
p1_tt = ['Confirm the features that will be usedto compute pairwise scores between same and different label samples!'];
...
p2_tt=['Apply the transformation to the selected features BEFORE the calculation of scores'];
...
p3_tt = ['compute pairwise distances between same-label sampels and different-label samples based on selected label level and using distance metric selected from drop down list'];
...
h.p4 = uicontrol('style','pushbutton','units','normalized',...
    ...
    'fontweight','b','fontsize',9,'callback',@p4_call);
